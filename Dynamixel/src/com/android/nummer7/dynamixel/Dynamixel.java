...
public class Dynamixel {
	 ...
	 public class PROTOCOL{
		 ...
		 public class ERR{
			 static final byte RESULT_FAIL		= (1);  //Failed to process the instruction packet.
			 static final byte INSTRUCTION		= (2);  //Instruction error
			 static final byte CRC				= (3);  //CRC check error
			 static final byte DATA_RANGE		= (4);  //Data range error
			 static final byte DATA_LENGTH		= (5);  //Data length error
			 static final byte DATA_LIMIT		= (6);  //Data limit error
			 static final byte ACCESS			= (7);  //Access error
		 }
		 ...
		 public class INST{
			 ...
			 static final short SYNC_WRITE		= (131);
			 static final short	BULK_READ      = (146);  // 0x92
		 }
	...
	public static byte getChecksum(byte[] data, int length){
		...
		for (int i=2; i<length; i++){
			Log.d("STUFF", "drivers: "+(char)data[i]);
			sum += (char)data[i];
		}
	...
	public static void dxl_ping( int id ) throws IOException
	{
		...
		gbInstructionPacket[PRT1_PKT_ID] = (char) id;
	...
	/*
	public static int dxl_read_byte( int id, int address ) throws IOException
	{
		gbInstructionPacket[PROTOCOL.PRT1_PKT.ID] = (byte) id;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.INSTRUCTION] = PROTOCOL.INST.READ;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.PARAMETER0+0] = (byte) address;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.PARAMETER0+1] = 1;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.LENGTH] = 4;
		dxl_txrx_packet();
		return (int)gbStatusPacket[PROTOCOL.PRT1_PKT.PARAMETER0];
	}

	public static void dxl_write_byte( int id, int address, int value ) throws IOException
	{
		gbInstructionPacket[PROTOCOL.PRT1_PKT.ID] = (byte) id;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.INSTRUCTION] =  PROTOCOL.INST.WRITE;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.PARAMETER0+0] = (byte) address;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.PARAMETER0+1] = (byte) value;
		gbInstructionPacket[PROTOCOL.PRT1_PKT.LENGTH] = 4;
		dxl_txrx_packet();
	}*/
